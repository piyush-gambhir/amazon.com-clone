...
export default function Carousel({ CarouselLinks }: Props) {
  ...
  return (
    <div className="relative overflow-hidden w-full h-[496px]">
      {CarouselLinks.map((CarouselLink, index) => (
        ...
        >
          <Image
            src={CarouselLink.image}
            alt={"carousel-image"}
            layout="fill"
            quality={100}
            className="object-cover object-center w-full h-full z-0 transition-opacity duration-500 ease-in-out "
          />
        </div>
      ))}
      <button
